...
const Overview = () => {
  ...
  const [last30DaysPrograms, setLast30DaysPrograms] = useState(0);
  ...
  // const { posts, couchData, subscribersData, postsData, loading } =
  // useContext(DataContext);
  const dropdownItems = [
    { label: "Post", href: "/dashboard/createPost", icon: RiMenu2Fill },
  ...
  const fetchPrograms = async () => {
    try {
      setLoading(true);
      const res = await fetch(`/api/formD`, {
        method: "GET",
      });
      const data = await res.json();
      if (data.success) {
  ...
  // Filter subscribers who subscribed in the last 30 days
  const filterLast30DaysPrograms = (programs) => {
    const thirtyDaysAgo = subDays(new Date(), 30);
    const filteredPrograms = subscribers.filter((programs) => {
      const programsDate = new Date(programs.subscriptionDate);
      return programsDate >= thirtyDaysAgo;
    });
    setLast30DaysPrograms(filteredPrograms.length);
  };
  ...
  useEffect(() => {
    if (programs && programs.length > 0) {
      filterLast30DaysPrograms(programs);
    }
  }, [programs]);
