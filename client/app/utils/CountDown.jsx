...
const Countdown = () => {
  ...
  useEffect(() => {
    const calculateCountdown = () => {
      ...
      if (now > THANK_YOU_END) return null;
      ...
      if (now >= EVENT_END && now <= THANK_YOU_END) {
        return {
          ...
          cta: "Subscribe to stay updated on future Coderina championships and STEAM programs.",
          phase: "thank-you",
        };
      }
      ...
      if (diffDays > 0 && diffDays <= 10) {
        ...
        // pick first valid image that exists
        const imageArray = IMAGE_EXTENSIONS.map(
          (ext) => `/count/day${diffDays}.${ext}`,
        );
        const image = imageArray[0]; // always default to first one
    ...
    const updateCountdown = () => {
      ...
      setImgSrc(data.image || data.fallbackImage);
    };
      ...
      <article className="relative w-[260px] 2xl:w-[380px] bg-gradient-to-br from-teal-800 via-teal-900 to-black rounded-2xl shadow-2xl overflow-hidden border border-teal-700/30 backdrop-blur-sm">
        ...
        <figure className="relative w-full h-36 2xl:h-56 overflow-hidden">
          <Image
            ...
            onError={() => setImgSrc(countdownData.fallbackImage)}
          />
        ...
        <div className="relative p-4 2xl:p-6 space-y-2 2xl:space-y-3">
          ...
          <div className="pt-3 border-t border-teal-700/30">
            <p className="text-xs text-teal-400/70">
              <span className="font-semibold">Coderina</span> National
              Championship 2026
            </p>
          </div>
        </div>
