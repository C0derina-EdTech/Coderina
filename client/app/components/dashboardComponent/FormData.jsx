...
const RegistrationsTable = () => {
  ...
  const fetchRegistrations = async () => {
    setLoading(true)
    try {
      const res = await fetch("/api/formData", { method: "GET" });
  ...
  return (
    <div className="w-full  md:px-2 lg:px-4 py-6 overflow-hidden h-full">
      ...
      <div>
        <h1 className="text-2xl font-bold mb-4">Couch Projects</h1>

        {loading ? (
        <p className="text-center py-4 h-screen">Loading Projects...</p>
      ) : registrations.length === 0 ? (
        <p className="text-center py-4 h-screen">No project found</p>
      ) : (
        <>
         <div className="mb-4 text-right font-medium">
          Total Projects: {registrations.length}
        </div>
